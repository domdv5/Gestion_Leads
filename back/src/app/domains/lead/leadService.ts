...
class LeadService {
    ...
    try {
      ...
      if (_params._id) {
        ...
        const create = await Lead.create({
          incremental: Math.floor(1000 + Math.random() * 9000),
          ..._params,
        });
      ...
      if (error.code === 11000) {
        return responseUtility.error("lead.duplicate", null, { code: 400 });
      }
  ...
  public async delete(id: string) {
    try {
      ...
      return responseUtility.success({ message: "Lead deleted" });
    } catch (error) {
      console.log("error", error);
      return responseUtility.error("Error deleting lead");
    }
  }
