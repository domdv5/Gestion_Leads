  ...
  try {
    ...
    if (_params?.options?.cache) {
      const exists = localStorage.getItem(
        `${app}:cache:get${_params.api}:${token.get()}`
      );
      if (exists) {
        const data = JSON.parse(exists);
        if (
          moment.utc().diff(moment.utc(data.updated_at), "minutes") <
          _params?.options?.cache
        ) {
...
export const post = async (_params: {
  api: string;
  options?: { data?: any; config?: any; cache?: number };
}) => {
  try {
    ...
    if (_params?.options?.cache) {
      const exists = localStorage.getItem(
        `${app}:cache:post:${_params.api}:${token.get()}`
      );
      if (exists) {
        const data = JSON.parse(exists);
        if (
